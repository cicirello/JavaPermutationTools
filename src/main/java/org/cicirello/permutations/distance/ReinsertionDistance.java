...
/**
 * Reinsertion distance is the count of the number of removal/reinsertion operations needed to
 * transform one permutation into the other.
 *
 * <p>This implementation utilizes the observation that the elements that must be removed and
 * reinserted are exactly those elements that are not in the longest common subsequence.
 *
 * <p>Runtime: O(n lg n), where n is the permutation length.
 *
 ...
 *
 * <p>However, in that paper, it was computed, in O(n^2) time, using an adaptation of string Edit
 * Distance.
 *
 ...
 *
 * <p>However, that paper used an O(n^2) time algorithm for longest common subsequence. This class
 * has been updated to use a more efficient O(n lg n) algorithm for longest common subsequence. It
 * is a version of Hunt et al's algorithm that has been optimized to assume permutations of the
 * integers in [0, (n-1)] with unique elements. The original algorithm of Hunt et al was for general
 * strings that could contain duplicates and which could consist of characters of any alphabet. In
 * that more general case, O(n lg n) was the best case runtime. In our special case, O(n lg n) is
 * worst case runtime.
 *
 * <p>See the following for complete details of Hunt et al's algorithm for longest common
 * subsequence:<br>
