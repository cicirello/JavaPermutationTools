...
/**
 * The original version of Normalized Deviation distance (Ronald, 1998) is the sum of the positional
 * deviation of the permutation elements divided by N-1 (where N is the length of the permutation).
 * The positional deviation of an element is the difference in its location in the two permutations.
 * Normalizing by dividing by N-1 causes each element's contribution to distance to be in the
 * interval [0,1].
 *
 ...
 * factor is (N<sup>2</sup>/2) when N is even and (N<sup>2</sup>-1)/2 when N is odd.
 *
 ...
 *
 * <p>The length is 6, which is even, so we'll divide by 18. So, normalized deviation distance is 8
 * / 18 = 0.444...
 *
 * <p>If instead, p2 = [5, 4, 3, 2, 1, 0], then 0 and 5 are both displaced by 5 positions, 1 and 4
 * are displaced by 3 positions, and 2 and 3 are displaced by 1 position. Sum of deviations is then:
 * 2 * 5 + 2 * 3 + 2 * 1 = 18. The length is still 6, so we again divide by 18, and distance is 1.
 *
