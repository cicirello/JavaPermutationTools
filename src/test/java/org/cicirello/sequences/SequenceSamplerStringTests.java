...
public class SequenceSamplerStringTests {
  ...
  private void validateSamples(Sampler sampler) {
    for (int n = 1; n < 9; n++) {
      ...
      for (int k = 1; k <= n; k++) {
        validateSample(allDiff, sampler.sample(sAllDiff, k, null), k);
        validateSample(mixedQuantities, sampler.sample(sMixed, k, null), k);
        validateSample(allSame, sampler.sample(sSame, k, null), k);
      }
    }
