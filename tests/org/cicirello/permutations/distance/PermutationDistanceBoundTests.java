...
/**
 * JUnit 4 tests for the bound/boundf methods of various classes that implement permutation distance metrics.
 */
public class PermutationDistanceBoundTests {
	...
	public void testAcyclicEdgeDistance() {
		...
		for (int n = 0; n <= 7; n++) {
			...
			assertEquals("Failed on length: " + n, expected, d.bound(n));
	...
	public void testCyclicEdgeDistance() {
		...
		for (int n = 0; n <= 7; n++) {
			int expected = bruteForceComputeMax(d,n);
			assertEquals("Failed on length: " + n, expected, d.bound(n));
			assertEquals("Failed on length: " + n, 1.0*expected, d.boundf(n), EPSILON);
		}
	}
	...
	public void testDeviationDistanceNormalized() {
		...
		for (int n = 0; n <= 7; n++) {
			assertEquals("Failed on length: " + n, bruteForceComputeMaxD(d,n), d.boundf(n), EPSILON);
		}
	}
		...
		for (double i = 0.0; i < 1.0; i += 0.1) {
			for (double d = 0.0; d < 1.0; d += 0.1) {
				for (double c = i+d; c < 1.0; c += 0.1) {
					...
					for (int n = 0; n <= 5; n++) {
						assertTrue("Failed on length " + n + " and (i,d,c) = (" + i + ", " + d + ", " + c + ")", bruteForceComputeMaxD(dist,n) <= dist.boundf(n));
					}
				}
			}
		}
